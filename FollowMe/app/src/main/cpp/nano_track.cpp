...
int NanoTrack::load_model(AAssetManager* mgr){

    const char * backbone_param = "models/NanoTrack/nanotrack_backbone_sim-opt.param";
    int status = this->net_backbone.load_param(mgr, backbone_param);
    if(status<0){
        LOGE("load param failed: %s", backbone_param);
        return status;
    }

    const char * backbone_model = "models/NanoTrack/nanotrack_backbone_sim-opt.bin";
    status = this->net_backbone.load_model(mgr, backbone_model);
    if(status<0){
        LOGE("load model failed: %s", backbone_model);
        return status;
    }

    const char * head_param = "models/NanoTrack/nanotrack_head_sim-opt.param";
    status = this->net_head.load_param(mgr, head_param);
    if(status<0){
        LOGE("load param failed: %s", head_param);
        return status;
    }

    const char * head_model = "models/NanoTrack/nanotrack_head_sim-opt.bin";
    status = this->net_head.load_model(mgr, head_model);
    if(status < 0){
        LOGE("load model failed: %s", head_model);
        return status;
    }
...
void NanoTrack::init(cv::Mat & img, cv::Rect & bbox)
{
...

    {
        ...
        ncnn::Mat ncnn_img = ncnn::Mat::from_pixels(z_crop.data,
                                                    ncnn::Mat::PIXEL_BGR2RGB,
                                                    z_crop.cols,
                                                    z_crop.rows);

        // NanoTrack跟踪器不需要使用均值和方差
...
void NanoTrack::update(const cv::Mat &x_crops,
                       cv::Point &target_pos,
                       cv::Point2f &target_sz,
                       float scale_z,
                       float &cls_score_max)
{
    ...
    ncnn::Mat ncnn_img = ncnn::Mat::from_pixels(x_crops.data,
                                                ncnn::Mat::PIXEL_BGR2RGB,
                                                x_crops.cols,
                                                x_crops.rows);

    // 我们训练的 NanoTrack 不需要计算均值和方差
    // ncnn_img.substract_mean_normalize(this->mean_vals, this->norm_vals);
    ...
    ex_head.set_num_threads(16); // 16

    // 注意,如果输入的大小不对会导致 ex_neck_head.extract 过程报错
    ex_head.input("input1", zf); // 8,8,1,48
    ex_head.input("input2", xf); // 16,16,1,48
    ...
    LOGI("pscore_window max score is: %.3f", pscore[r_max * cols + c_max]);
...
float NanoTrack::track(cv::Mat const & im, cv::Rect & bbox) {
    ...
    LOGI("track target_sz: (%.1f, %.1f)", target_sz.x, target_sz.y);
    LOGI("track cls_score_max: %.3f", cls_score_max);
