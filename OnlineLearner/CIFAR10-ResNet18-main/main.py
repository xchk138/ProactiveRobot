...
case_id = 1
name_dataset = ['CIFAR10', 'Fashion-MNIST']
dataset_path = ['data/cifar-10-python', 'data/fashion-mnist']
ckpt_name = ['cifar10', 'fashion-mnist']
crop_size = [32, 32]
pad_size = [4, 6]
input_channel=[3, 1]
im_mean = [(0.4914, 0.4822, 0.4465), (0.2860405969887955)]
im_std = [(0.2023, 0.1994, 0.2010), (0.3530242445149223)]



parser = argparse.ArgumentParser(description=name_dataset[case_id] + ' Training')
...
transform_train = transforms.Compose([
    transforms.RandomCrop(crop_size[case_id], padding=pad_size[case_id]),
    transforms.RandomHorizontalFlip(),
    transforms.ToTensor(),
    transforms.Normalize(im_mean[case_id], im_std[case_id])
])
...
if case_id==0:
    ...
    trainset = torchvision.datasets.FashionMNIST(
        root='data/fashion-mnist', train=True, download=False, transform=transform_train)
    testset = torchvision.datasets.FashionMNIST(
        root='data/fashion-mnist', train=False, download=False, transform=transform_test)
...
# net = VGG('VGG19')
net = ResNet18(input_channel=input_channel[case_id])
# net = PreActResNet18()
# net = GoogLeNet()
# net = DenseNet121()
# net = ResNeXt29_2x64d()
...
if args.resume:
    ...
    checkpoint = torch.load('./checkpoint/'+ ckpt_name[case_id] +'.pth')
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/'+ ckpt_name[case_id] +'.pth')
        best_acc = acc
...
def plot_train_log():
    checkpoint = torch.load('./checkpoint/' + ckpt_name[case_id] +'.pth')
    best_acc = checkpoint['acc']
    train_loss, train_acc, test_loss, test_acc = checkpoint['train_history']
    visualize(train_loss, train_acc, test_loss, test_acc, 'SGD')
    print(best_acc)
...
if __name__=='__main__':
    ...
    #      test(epoch)
    #      scheduler.step()
    # show the training curve and the best accuracy
